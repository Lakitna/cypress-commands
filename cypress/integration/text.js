    ...
    describe('The option `depth`', function () {
        it('`0` is the default value', function () {
            cy.get('div.parent')
                ...
                .should(
                    'equal',
                    ['parent div top', 'parent div middle', 'parent div bottom'].join(' ')
                );
        });
        ...
        it('gets all values of form elements', function () {
            cy.get('form').text({ depth: 1 }).should('equal', '5 A filled textarea Some button');
        });
    });
    ...
    describe('errors', function () {
        ...
        it('is called as a parent command', function (done) {
            cy.on('fail', (err) => {
                ...
                expect(err.message).to.include(
                    'Oops, it looks like you are trying to call a child ' +
        ...
        it('not preceded with an element', function (done) {
            cy.on('fail', (err) => {
                ...
                expect(lastLog.get('error')).to.eq(err);
                expect(err.message).to.include(
                    '`cy.text()` failed because it requires a DOM element.'
                );
                done();
            });
        ...
        it('wrong value for the option `depth`', function (done) {
            cy.on('fail', (err) => {
                ...
                expect(lastLog.get('error')).to.eq(err);
                expect(err.message).to.include(
                    'Bad value for the option "depth" of the command "text"'
                );
                done();
            });

            cy.get('#foo').text({ depth: -1 });
        });
